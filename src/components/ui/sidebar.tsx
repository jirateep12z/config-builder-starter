...
import type { SidebarMenuButtonVariantsProps } from '@/components/ui/sidebar-variants';
import { sidebar_menu_button_variants } from '@/components/ui/sidebar-variants';
...
function SidebarRail({ className, ...props }: React.ComponentProps<'button'>) {
  ...
  return (
    <button
      ...
      className={Cn(
        'hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] sm:flex ltr:-translate-x-1/2 rtl:-translate-x-1/2',
...
function SidebarInset({ className, ...props }: React.ComponentProps<'main'>) {
  return (
    <main
      ...
      className={Cn(
        'bg-background relative flex w-full flex-1 flex-col md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2',
        className
      )}
      {...props}
    />
...
function SidebarMenuButton({
  ...
  const button = (
    <Comp
      ...
      className={Cn(sidebar_menu_button_variants({ variant, size }), className)}
      {...props}
    />
  );
