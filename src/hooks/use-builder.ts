...
function BuildStateFromFramework(framework: FrameworkConfig): BuilderState {
  return {
    ...
    ignore_patterns: [
      ...new Set([...framework.prettier_ignore, ...DEFAULT_IGNORE_PATTERNS])
    ],
    custom_patterns: []
  };
}
...
export function UseBuilder() {
  ...
  const output: GeneratedOutput = useMemo(
    () => ({
      ...
      gitignore: LibGenerateGitignore(
        state.ignore_patterns,
        state.custom_patterns,
        selected_framework_data
      ),
  ...
  const GetIsEslintFormatAvailable = useCallback(
    (format: EslintFormat) =>
      !selected_framework_data?.unsupported_eslint_formats?.includes(format),
    [selected_framework_data]
  );
