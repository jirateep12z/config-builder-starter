import type { EslintFormat, FrameworkIconId } from './builder';

export type EslintFlatPlugin = {
  import_name: string;
  from: string;
  configs: string[];
  plugin_key?: string;
...
export type EslintFlatOptions = {
  imports?: string[];
  configs?: string[];
  plugins?: EslintFlatPlugin[];
  files_pattern?: string;
  config_template?: (rules_str: string) => string;
  config_block?: (rules_str: string) => string;
  extra_blocks?: (rules_str: string) => string;
};
...
export type FrameworkConfig = {
  ...
  eslint_flat?: EslintFlatOptions;
  unsupported_eslint_formats?: EslintFormat[];
